...
class ReelService {
  ...
  static final userReelsStreamProvider = StreamProvider<List<ReelModel>>((ref) {
    ...
    return FirebaseFirestore.instance
        ...
        .map((snapshot) {
          print(
            "Total documents found in Firestore: ${snapshot.docs.length}",
          ); // Debug log
          ...
          return snapshot.docs.map((doc) {
            ...
            return ReelModel(
              videoUrl: short.videoUrl,
              username:
                  'User_${short.userId.length > 5 ? short.userId.substring(0, 5) : short.userId}',
              caption: short.caption,
              musicName: 'Original Audio',
              likes: short.likesCount,
              comments: short.commentsCount,
    ...
    try {
      ...
      return snapshot.docs.map((doc) {
        ...
        return ReelModel(
          videoUrl: short.videoUrl,
          username:
              'User_${short.userId.length > 5 ? short.userId.substring(0, 10) : short.userId}',
          caption: short.caption,
          musicName: 'Original Audio',
          likes: short.likesCount,
          comments: short.commentsCount,
  ...
  static final otherUserReelsStreamProvider =
      StreamProvider.family<List<ReelModel>, String>((ref, userId) {
        return FirebaseFirestore.instance
            .collection('shorts')
            ...
            .map((snapshot) {
              return snapshot.docs.map((doc) {
                ...
                return ReelModel(
                  ...
                  shares: short.viewsCount,
                  profileUrl:
                      short.videoUrl, // Passing video url for ReelGridItem
                  isLiked: false,
                  userID: short.userId,
                );
              }).toList();

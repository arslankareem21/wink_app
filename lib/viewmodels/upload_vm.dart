...
//     try {
//       final upload = await _cloudinary.uploadFile(file: , isVideo: );
...
//   //     final videoUpload = await _cloudinary.uploadFile(file: file, isVideo: isVideo)
//   //     // _cloudinary.uploadFile(
//   //     //   file: video,
//   //     //   type: 'shorts',
//   //     //   userId: userId,
//   //     //   onProgress: (p) =>
//   //     //       state = state.copyWith(progress: p * 0.7), // 70% for video
//   //     // );
...
//   //       userId: userId,
//   //       onProgress: (p) =>
//   //           state = state.copyWith(progress: 0.7 + (p * 0.3)), // 30% for thumb
//   //     );
...
//     try {
//       final upload = await _cloudinary.uploadFile(file: file, isVideo: isVideo);
//       // _cloudinary.uploadFile(
//       //   file: file,
//       //   type: 'stories',
//       //   userId: userId,
//       //   onProgress: (p) {
//       //     // Guard against updating disposed state
//       //     if (mounted) {
...
//       if (mounted) state = state.copyWith(status: 'Saving...');
//       await _firestore.saveStory(story);
...
//         );
//       }
//       rethrow; // Rethrow so UI try-catch can catch it
//     }
//   }


//   // Future<void> uploadProfilePic({{required File file, required bool isVideo}
//   //   //required File file, required bool isVideo
//   //   }) async {
//   //   if (state.isUploading) return;
...
//     StateNotifierProvider.autoDispose<UploadViewModel, UploadState>((ref) {
  ...
  Future<void> uploadShort({required File video, required String caption}) async {
    ...
    try {
      ...
      final short = ShortModel(
        ...
        commentsCount: 0,
        viewsCount: 0,
        createdAt: DateTime.now(), thumbnailUrl: '',
      );
